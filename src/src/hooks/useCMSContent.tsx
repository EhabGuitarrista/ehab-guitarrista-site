    ...
    const loadContent = async () => {
      try {
        ...
        if (response.ok) {
          ...
          console.log('🔍 Video URL present:', !!(data.videos?.aboutSection?.file));
          setContent({ ...defaultContent, ...data });
        } else {
          console.log('Failed to load CMS content, using defaults');
          setContent(defaultContent);
        }
      } catch (err) {
        console.log('Error loading CMS content, using defaults:', err);
